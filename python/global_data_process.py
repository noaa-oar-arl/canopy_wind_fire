...
metlist = ['ugrd10m', 'vgrd10m', 'fricv', 'sfcr', 'vtype', 'sotyp', 'pressfc', \
           'dswrf', 'shtfl', 'tmpsfc', 'tmp2m', 'spfh2m', 'hpbl', 'prate_ave']
...
def mapping(xgrid, ygrid, data, xdata, ydata, map_method, fvalue):
    output = griddata((xdata, ydata), data, (xgrid, ygrid), method=map_method, fill_value=fvalue)
    return output
...
def read_frp_local(filename, lat, lon, fill_value):
    ...
    DATA = mapping(lat, lon, data.flatten(), yt.flatten(), xt.flatten(), 'nearest', fill_value)
    return DATA
...
for inputtime in timelist:
    ...
    if (len(inputtime) != 13) or (int(inputtime[4:6]) > 12) or (int(inputtime[6:8]) > 31) or (int(inputtime[8:10]) > 23) or (int(inputtime[10:]) > 23):
    ...
    ## input/output files
    f_met    = path+'/gfs.t'+HH+'z.'+YY+MM+DD+'.sfcf'+FH+'.nc'                  # gfs met file
    f_can    = path+'/gfs.canopy.t'+HH+'z.'+'2022'+MM+DD+'.sfcf000.global.nc'   # canopy file
    f_output = path+'/gfs.t'+HH+'z.'+YY+MM+DD+'.sfcf'+FH+'.canopy.nc'           # output file
    
    if frp_src == 0:  # local frp file
        if int(YY+MM+DD) <= 20230510:     # version 3
            f_frp = '/groups/ESS/yli74/data/GBBEPx/ORI/GBBEPx_all01GRID.emissions_v003_'+YY+MM+DD+'.nc'
        else:                             # version 4
    ...
    for i in np.arange(len(canlist)):
        ...
        if varname == 'lai':
            ...
            time_conv = datetime(int(YY), int(MM), int(DD), int(HH), 0, 0, 0, tzinfo=timezone.utc) \
        ...
        if varname == 'pavd':
            ...
            var     = output.createVariable(varname, 'float', ('time', 'level', 'grid_yt', 'grid_xt'), fill_value=fill_value)
    
            write_varatt(var, ATTNAME, ATT)
            write_varatt(var_lev, ['long_name', 'units'], ['height of the layer midpoint above the ground', 'm'])
            ...
            var = output.createVariable(varname, 'float', ('time', 'grid_yt', 'grid_xt'), fill_value=fill_value)
            write_varatt(var, ATTNAME, ATT)
            var[:] = DATA
